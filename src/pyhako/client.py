...
class Client:
    ...
    def __init__(
        ...
        if use_token_storage:
            try:
                self.token_manager = TokenManager(use_keyring=True)
    ...
    async def refresh_access_token(self, session: aiohttp.ClientSession) -> bool:
        ...
        """
        if not self.cookies:
            logger.debug("No cookies available for refresh.")
            return False

        url = f"{self.api_base}/update_token"
        try:
            async with session.post(url, headers=self.headers, json={}, ssl=False) as resp:
                if resp.status == 200:
                    ...
                    logger.warning(f"Token refresh failed with status {resp.status}")
        except Exception as e:
